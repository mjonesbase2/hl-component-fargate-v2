...
describe 'compiled component fargate-v2' do
  ...
  context 'cftest' do
    it 'compiles test' do
      expect(system("cfhighlander cftest #{@validate} --tests tests/availability_zone_rebalancing.test.yaml")).to be_truthy
    end      
  end
  
  let(:template) { YAML.load_file("#{File.dirname(__FILE__)}/../out/tests/availability_zone_rebalancing/fargate-v2.compiled.yaml") }
  ...
  context "Resource" do
    ...
    context "EcsFargateService" do
      ...
      it "to have property AvailabilityZoneRebalancing" do
          expect(resource["Properties"]["AvailabilityZoneRebalancing"]).to eq("ENABLED")
      end
